...
class _ProfileEditScreenState extends State<ProfileEditScreen>  with TickerProviderStateMixin {
  ...
  _updateUserAccount() async {
    ...
    else if(Get.find<ProfileController>().passwordController.text.isNotEmpty && Get.find<ProfileController>().passwordController.text.length <8){
      showCustomSnackBar('password_be_at_least_8_character'.tr);
    }else if(Get.find<ProfileController>().passwordController.text.isNotEmpty  && Get.find<ProfileController>().passwordController.text != Get.find<ProfileController>().confirmPasswordController.text){
          ...
          return ListView( children: [
              ...
              Padding(padding:  EdgeInsets.symmetric(horizontal:Dimensions.paddingSizeDefault),
                child: Container(width: MediaQuery.of(context).size.width,
                  ...
                  CustomButton(onTap: _updateUserAccount, btnTxt: 'update_profile'.tr) :
